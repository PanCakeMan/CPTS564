...
/**
* A sequence of Vehicle instances.
*/
public final class VehicleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public VehicleSeq (int initialMaximum) {
        super(Vehicle.class, initialMaximum);
    }

    public VehicleSeq (Collection elements) {
        super(Vehicle.class, elements);
    }

    public  Vehicle get(int index) { 
        return (Vehicle) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, Vehicle.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' Vehicle objects (beyond the original size of this object)
